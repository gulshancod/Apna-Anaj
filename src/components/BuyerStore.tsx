...
import { ProductItem, ProductCategory, LanguageCode } from '../types';
...
export const BuyerStore: React.FC<BuyerStoreProps> = ({
  ...
  const openPricePopup = async (productName: string) => {
    ...
    try {
      ...
      setPricePopup({ productName, loading: false, data: result, error: '' });
    } catch (error: any) {
      setPricePopup({ productName, loading: false, data: null, error: error?.message || 'Unable to load mandi price data.' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 15-Min Express Delivery Banner */}
      <div className="rounded-3xl bg-[#1e5634] text-white p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-lg shadow-[#1e5634]/15 relative overflow-hidden">
        <div className="space-y-1.5 z-10">
          ...
          <p className="text-xs sm:text-sm text-white/90 flex items-center gap-1.5">
            ...
            <span>Delivering to: <strong className="text-[#f7c244]">{buyerAddress || 'Ghaziabad, Uttar Pradesh'}</strong></span>
          </p>
        </div>
      ...
      {filteredProducts.length === 0 ? (
        <div className="text-center py-16 p-8 rounded-3xl bg-white dark:bg-[#15271e] border border-[#e5dec9] dark:border-[#223f30]">
          ...
          <h3 className="font-heading text-lg font-bold text-[#1f3427] dark:text-[#f4f8f5]">No crops match "{searchQuery}"</h3>
          <p className="text-xs text-[#5e7164] dark:text-[#9ab0a2] mt-1">Try searching for Gehu, Rice, Chana, Bajra, Tomato or Palak</p>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {filteredProducts.map((prod) => {
            ...
            const isMandiAvailable = typeof mandiPrice === 'number' && Number.isFinite(mandiPrice);
            const displayedPrice = isMandiAvailable ? mandiPrice : prod.price;
            const pricedProduct = isMandiAvailable ? { ...prod, price: mandiPrice } : prod;
            ...
            return (
              ...
              >
                ...
                <div className="relative h-44 w-full bg-[#e4f1cd] dark:bg-[#163824] overflow-hidden">
                  ...
                  >
                    <Heart className={`w-4 h-4 ${isFav ? 'fill-[#f28b47] text-[#f28b47]' : ''}`} />
                  </button>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {prod.tags.slice(0, 2).map((tag, idx) => (
                        <span key={idx} className="text-[10px] font-medium px-2 py-0.5 rounded bg-[#e4f1cd] dark:bg-[#163824] text-[#1e5634] dark:text-[#4ade80]">
                          {tag}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="pt-2 border-t border-[#e5dec9]/60 dark:border-[#223f30] flex items-center justify-between">
                    ...
                    {currentQty === 0 ? (
                      <button
                        ...
                        className={`flex items-center gap-1 px-3.5 py-1.5 rounded-lg border-1.5 border-[#1e5634] bg-white dark:bg-[#15271e] text-[#1e5634] dark:text-[#4ade80] hover:bg-[#1e5634] hover:text-white text-xs font-bold shadow-sm transition-all ${isMandiAvailable ? 'cursor-pointer' : 'opacity-50 cursor-not-allowed'}`} disabled={!isMandiAvailable}
                      >
      ...
      {pricePopup && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setPricePopup(null)}>
          <div className="w-full max-w-sm rounded-2xl bg-white dark:bg-[#15271e] border border-[#e5dec9] dark:border-[#223f30] shadow-2xl p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[10px] font-extrabold uppercase tracking-wider text-[#5e7164] dark:text-[#9ab0a2]">Government Mandi Price</p>
                <h3 className="font-heading text-lg font-bold text-[#1f3427] dark:text-white mt-0.5">{pricePopup.productName}</h3>
              </div>
              <button type="button" onClick={() => setPricePopup(null)} className="w-7 h-7 rounded-full bg-[#eef3ec] dark:bg-[#223f30] text-[#5e7164] dark:text-[#9ab0a2] font-bold cursor-pointer" aria-label="Close">×</button>
            </div>

            {pricePopup.loading ? (
              <div className="py-8 text-center text-sm text-[#5e7164] dark:text-[#9ab0a2]">Loading government mandi data...</div>
            ) : pricePopup.data ? (
              <div className="mt-5 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-xl bg-[#eef6e8] dark:bg-[#163824] p-3"><div className="text-[10px] font-bold text-[#5e7164] dark:text-[#9ab0a2]">CURRENT</div><div className="text-xl font-black text-[#1e5634] dark:text-[#4ade80] mt-1">₹{pricePopup.data.currentPricePerKg}/kg</div></div>
                  <div className="rounded-xl bg-[#f7f1e4] dark:bg-[#2a3026] p-3"><div className="text-[10px] font-bold text-[#5e7164] dark:text-[#9ab0a2]">LAST MONTH</div><div className="text-xl font-black text-[#1f3427] dark:text-white mt-1">₹{pricePopup.data.lastMonthPricePerKg}/kg</div></div>
                </div>
                <div className="rounded-xl border border-[#e5dec9] dark:border-[#223f30] p-3 flex items-center justify-between">
                  <div><div className="text-[10px] font-bold text-[#5e7164] dark:text-[#9ab0a2]">PRICE CHANGE</div><div className="text-sm font-bold text-[#1f3427] dark:text-white mt-1">{pricePopup.data.changePercent > 0 ? '+' : ''}{pricePopup.data.changePercent}%</div></div>
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black ${pricePopup.data.priceLevel === 'HIGH' ? 'bg-[#fee2e2] text-[#b91c1c]' : pricePopup.data.priceLevel === 'LOW' ? 'bg-[#dcfce7] text-[#166534]' : 'bg-[#fef3c7] text-[#92400e]'}`}>{pricePopup.data.priceLevel}</span>
                </div>
                <div className="text-[10px] leading-relaxed text-[#6b7b70] dark:text-[#9ab0a2]">Source: Government of India – AGMARKNET<br />Current date: {pricePopup.data.currentDate}<br />Comparison date: {pricePopup.data.lastMonthDate}</div>
              </div>
            ) : (
              <div className="mt-5 rounded-xl bg-[#fff7ed] dark:bg-[#2b241b] border border-[#fed7aa] p-4"><p className="text-sm font-semibold text-[#9a3412] dark:text-[#fdba74]">Mandi comparison unavailable</p><p className="text-xs text-[#9a3412]/80 dark:text-[#fdba74]/80 mt-1">{pricePopup.error}</p></div>
            )}
          </div>
        </div>
      )}
